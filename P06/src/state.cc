 ...
 */
State::State(const std::vector<std::string>& properties) {
  ...
  for (unsigned i = 3; i < properties.size(); i += 2) {
    int j = i + 1;
    std::pair<char, std::string> transition(properties[i][0], properties[j]);
    transitions_.insert(transition);
  }
}

/** @brief Constructor de copia de la clase State
 *  @param[in] state. Estado a copiar
 */
State::State(const State& state) {
  state_id_ = state.state_id_;
  alphabet_ = state.alphabet_;
  final_ = state.final_;
  number_of_transitions_ = state.number_of_transitions_;
...
bool operator==(const State& s1, const State& s2) {
  if (s1.state_id_ != s2.state_id_ || s1.final_ != s2.final_ || s1.alphabet_ != s2.alphabet_ || 
      s1.number_of_transitions_ != s2.number_of_transitions_ || s1.transitions_ != s2.transitions_) {
    return false;
  }
  return true;
}